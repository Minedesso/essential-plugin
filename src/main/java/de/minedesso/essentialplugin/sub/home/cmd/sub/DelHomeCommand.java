...
public class DelHomeCommand implements SubCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        if(args.length != 1) {
            sender.sendMessage(Message.USAGE.message + "/delhome <name>");
            return;
        }
        ...
        try {
            ...
            HomeService.getInstance().deleteHome(player.getUniqueId(), homeName);
            player.sendMessage(Message.PREFIX.message + "Home '" + homeName + "' deleted successfully.");
